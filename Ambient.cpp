 ...
 */
#include "Ambient.h" //#はプリプロセッサ命令文。コンパイル時に与える

#define AMBIENT_DEBUG 0

#if AMBIENT_DEBUG  //AMBIENT_DEBUGがゼロ以外の場合
...
bool
//Ambient::begin(unsigned int channelId, const char * writeKey, WiFiClient * c, int dev) {  //WiFiClient * c
Ambient::begin(unsigned int channelId, const char * writeKey, EthernetClient * c, int dev) {
    this->channelId = channelId;  //thisポインタ。このAmbientクラスのchannelIDに引数のchannelIDの中身を渡す

    if (sizeof(writeKey) > AMBIENT_WRITEKEY_SIZE) {  //writeKeyのバイト数が18以上の場合
        ERR("writeKey length > AMBIENT_WRITEKEY_SIZE");
        return false;
    }
    strcpy(this->writeKey, writeKey); //strcpy(char *dest, char *src)はdestに指定したアドレスの文字列をコピー
...
Ambient::send() {
    ...
    //Ambient.ioに接続。指定したリトライ回数まで失敗するとエラーを返す
    int retry;
    for (retry = 0; retry < AMBIENT_MAX_RETRY; retry++) {
        int ret;
        //client.connect(server,port)は指定したipアドレス、ポートに接続。成功したら1を返す
